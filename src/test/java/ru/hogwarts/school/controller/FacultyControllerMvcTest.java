...
class FacultyControllerMvcTest {
    ...
    public void getFacultyByNameOrByColor() throws Exception {
        when(facultyRepository.findAllByNameOrColorIgnoreCase(anyString(), anyString())).thenReturn(MOCK_FACULTIES);

        mockMvc.perform(MockMvcRequestBuilders
                        .get("/faculty/byColorAndName?name=" + MOCK_FACULTY_NAME + "&color=" + MOCK_FACULTY_COLOR)
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
